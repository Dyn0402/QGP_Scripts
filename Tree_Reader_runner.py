...
def run_jobs():
    ...
    for energy in energies:
        for job_type, job_type_dict in jobs[energy].items():
            for set_group_name, set_group_dict in job_type_dict.items():
                for set_name, job_nums in set_group_dict.items():
                    command = f'export LD_LIBRARY_PATH={root_lib_path};' \
                              f'{exe_path} {energy} {job_type} {set_group_name} {set_name} ' \
                              f'{job_nums[0]} {job_nums[1]};read'
                    subprocess.Popen(['gnome-terminal', '--geometry=140x20', '--', 'bash', '-c', command])
