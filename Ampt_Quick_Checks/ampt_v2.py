...
def calculate_v2():
    ...
    plot_out_dir = 'C:/Users/Dylan/Desktop/'
    ...
    for energy in energies:
        ...
        jobs = [(f'{file_dir}{path}', read_branches, cents, ref3_edges, pid, max_rapid, min_pt, max_pt, max_p,
                 calc_quantities) for path in file_paths]

        v2_data = {cent: calc_quantities.copy() for cent in cents}
        with Pool(threads) as pool:
            for file_v2_data in tqdm.tqdm(pool.istarmap(read_file, jobs), total=len(jobs)):
                for cent in cents:
                    for quant in v2_data[cent]:
                        v2_data[cent][quant] += file_v2_data[cent][quant]

        v2_avgs, v2_sds, v2_avg_err, v2_rp_avgs, v2_rp_sds, v2_rp_avg_err, cent_labels = [], [], [], [], [], [], []
        reses, res_err = [], []
        for cent in cents:
            if v2_data[cent]['n_v2'] <= 0:
                continue
            res = np.sqrt(v2_data[cent]['ep_res_sum'] / v2_data[cent]['n_psi'])
            reses.append(res)
            res_sum_err = np.sqrt(v2_data[cent]['ep_res_sum2'] / v2_data[cent]['n_psi'] - res ** 4) / \
                          np.sqrt(v2_data[cent]['n_psi'])
            res_err.append(res_sum_err / (2 * res))  # Error propagation for sqrt
            # Assume no error for now, figure out how to deal properly later
            v2_avgs.append(v2_data[cent]['v2_ep_sum'] / v2_data[cent]['n_v2'] / res)
            v2_sds.append(np.sqrt(v2_data[cent]['v2_ep_sum2'] / v2_data[cent]['n_v2'] - v2_avgs[-1] ** 2) / res)
            v2_avg_err.append(v2_sds[-1] / np.sqrt(v2_data[cent]['n_v2']))
            v2_rp_avgs.append(v2_data[cent]['v2_rp_sum'] / v2_data[cent]['n_v2'])
            v2_rp_sds.append(np.sqrt(v2_data[cent]['v2_rp_sum2'] / v2_data[cent]['n_v2'] - v2_rp_avgs[-1] ** 2) / res)
            v2_rp_avg_err.append(v2_rp_sds[-1] / np.sqrt(v2_data[cent]['n_v2']))
        ...
        ax.legend()
        ax.set_title(f'AMPT {energy} GeV Elliptic Flow')
        fig.tight_layout()

        if plot_out_dir:
            fig.savefig(f'{plot_out_dir}Ampt_{energy}GeV_v2.png')
            ax.set_ylim(bottom=-0.01, top=0.1)
            fig.savefig(f'{plot_out_dir}Ampt_{energy}GeV_v2_zoom.png')
...
def read_file(file_path, read_branches, cents, ref3_edges, pid, max_rapid, min_pt, max_pt, max_p, calc_quantities):
    vector.register_awkward()
    v2_data = {cent: calc_quantities.copy() for cent in cents}
    # data_nproton = {cent: {n: {'sum': 0, 'sum2': 0, 'n': 0} for n in range(100)} for cent in cents}
    ep_pids = [211, 321, -211, -321]  # Just use pions and kaons for now
    ...
    with uproot.open(file_path) as file:
        ...
        for cent in cents:
            ...
            non_protons = []
            for ep_pid in ep_pids:  # Probably a columnar way to do this but dimension is too high for my head
            ...
            v2_data[cent]['v2_ep_sum'] = ak.sum(v2)
            v2_data[cent]['v2_ep_sum2'] = ak.sum(v2 ** 2)
            v2_data[cent]['v2_rp_sum'] = ak.sum(v2_rp)
            v2_data[cent]['v2_rp_sum2'] = ak.sum(v2_rp ** 2)
            v2_data[cent]['n_v2'] = ak.count(v2)
            v2_data[cent]['ep_res_sum'] = ak.sum(psi_res)
            v2_data[cent]['ep_res_sum2'] = ak.sum(psi_res**2)
            v2_data[cent]['n_psi'] = ak.count(psi_res)
