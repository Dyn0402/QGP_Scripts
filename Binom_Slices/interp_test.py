...
def chi2_sd_slices_bs():
    ...
    data_sets = ['bes', 'ampt_new_coal']
    data_sets_colors = dict(zip(data_sets, ['black', 'red']))
    data_sets_labels = dict(zip(data_sets, ['STAR', 'AMPT']))
    ...
    for data_set in data_sets:
        ...
        for energy_mins, energy_mins_bss, spreads in \
            ...
            avg_min[data_set].append(np.ma.average(energy_mins_avg, weights=weights))
        plt.axhline(1, color='black', ls='--')
        print(energies)
...
def chi2_sd_slices_bs_clean():
    pars = Pars()  # Define parameters for analysis
    datasets_data = DatasetsData()  # Initialize data containers for combined data sets
    ...
    for data_set in pars.data_sets:
        ...
        for energy in pars.energies:
            ...
            if 'group_2d' in pars.plot or 'indiv_2d' in pars.plot:
                plot_2ds(energy, x, y, X, Y, Z, f, bas_res, pars.plot, energies_data)

            energies_data = spread_1d_interps(df, data_set, energy, pars, f, f_jac, f_jac_bss, bs_cols, bas_res,
                                              energies_data)

        plot_energy_mins(data_set, pars, energies_data)
        datasets_data = plot_energy_mins_fit(data_set, pars, energies_data, datasets_data)
        datasets_data = plot_mins_over_avg(pars, energies_data, datasets_data)
...
class Pars:
    def __init__(self):
        ...
        self.data_sets = ['bes', 'ampt_new_coal']
        self.data_sets_colors = dict(zip(self.data_sets, ['black', 'red']))
        self.data_sets_labels = dict(zip(self.data_sets, ['STAR', 'AMPT']))
        ...
        self.plot = []  # 'indiv_2d', 'group_2d', 'vs_spread_indiv', 'vs_amp_indiv'
...
def plot_2ds(energy, x, y, X, Y, Z, f, bas_res, plot, energies_data):
    x_intp = np.linspace(min(x), max(x), 200)
    y_intp = np.linspace(min(y), max(y), 200)
    X_intp, Y_intp = np.meshgrid(get_edges(x_intp), get_edges(y_intp))
    Z_intp = []
...
def spread_1d_interps(df, data_set, energy, pars, f, f_jac, f_jac_bss, bs_cols, bas_res, energies_data):
    spreads = np.unique(df['spread'])
    ...
    energies_data.amp_min_data[data_set][energy] = {'spreads': spreads, 'mins': energies_data.amp_mins_per_spread[-1],
...
def plot_energy_mins(data_set, pars, energies_data):
    ...
    plt.title(data_set)
    color = iter(cm.rainbow(np.linspace(0, 1, len(pars.energies))))
    for x, y, am, ame, es, e in \
            zip(energies_data.min_amps, energies_data.min_spreads, energies_data.amp_mins_per_spread,
                energies_data.amp_min_sds_per_spread, energies_data.energy_spreads, pars.energies):
        c = next(color)
        am, ame = np.array(am), np.array(ame)
        plt.scatter(x, y, marker='+', color=c)
        plt.annotate(f'{e}', (x, y), textcoords='offset points', xytext=(0, 10), ha='center', color=c)
    ...
    for spread_index in range(len(energies_data.energy_spreads[0])):
        ...
        for energy_mins in energies_data.amp_mins_per_spread:
            e_sum += energy_mins[spread_index]
        energies_data.avgs_per_spread.append(e_sum / len(pars.energies))
...
def plot_energy_mins_fit(data_set, pars, energies_data, datasets_data):
    fig_mins_fit = plt.figure()
    fig_mins_fit.canvas.manager.set_window_title(f'Mins in Amp-Spread Space with Parameterization')
    plt.grid()
    # plt.xlim((0, 0.05))
...
def plot_mins_over_avg(pars, energies_data):
    ...
    color = iter(cm.rainbow(np.linspace(0, 1, len(pars.energies))))
    for energy_mins, energy_mins_sd, energy, spreads in \
            zip(energies_data.amp_mins_per_spread, energies_data.amp_min_sds_per_spread, pars.energies,
...
def plot_avg_mins_over_avg(data_set, pars, energies_data, datasets_data):
    ...
    plt.errorbar(pars.energies, datasets_data.avg_norm_min[data_set], yerr=datasets_data.avg_norm_min_sd[data_set],
                 marker='o', ls='none')
    fig_avg_mins_over_avg.tight_layout()
...
def plot_mins_vs_energy(data_set, pars, energies_data):
    ...
    for spread, min_amps in zip(spreads, inv_amp_mins):
        if spread > max_spread:
            continue
        c = next(color)
        plt.plot(pars.energies, min_amps, marker='o', color=c, alpha=0.7, label=f'spread {spread:.2f}')
    fig_mins_vs_e.legend()
    fig_mins_vs_e.tight_layout()
...
def plot_avg_norm_mins(pars, datasets_data):
    ...
    for data_set in pars.data_sets:
        plt.errorbar(pars.energies, datasets_data.avg_norm_min[data_set], yerr=datasets_data.avg_norm_min_sd[data_set],
                     marker='o', ls='none', label=data_set.upper())
    plt.legend()
    fig_avg_norm_mins.tight_layout()
...
def plot_avg_mins(pars, datasets_data):
    ...
    plt.axhline(0, color='black', ls='--')
    for data_set in pars.data_sets:
        plt.errorbar(pars.energies, datasets_data.avg_min[data_set], yerr=datasets_data.avg_min_sd[data_set],
                     marker='o', ls='none', label=pars.data_sets_labels[data_set],
                     color=pars.data_sets_colors[data_set])
    plt.legend()
    fig_avg_mins.tight_layout()
...
def plot_fit_base(pars, datasets_data):
    ...
    plt.axhline(0, color='black', ls='--')
    for data_set in pars.data_sets:
        plt.errorbar(pars.energies, datasets_data.baselines[data_set], yerr=datasets_data.baselines_sd[data_set],
                     marker='o', ls='none', label=data_set.upper())
    plt.legend()
    fig_fit_base.tight_layout()
