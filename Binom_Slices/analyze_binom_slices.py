...
def main():
    ...
    data_sets_plt = ['bes_resample_def', 'ampt_resample_def'] + sim_sets[:3]  # , 'ampt_def_resample']  # 'sim_aclmul_amp01_spread2', 'sim_aclmul_amp02_spread2'
    all_sets_plt = data_sets_plt + sim_sets

    df = pd.read_csv(df_path)
    df = df.dropna()
    # df_data = df[~df['data_set'].str.contains('sim_')]
    # df_sim = df[df['data_set'].str.contains('sim_')]
    ...
    for data_type in data_types:
        for data_set in data_sets_plt:
            for energy in energies:
                ...
                data.append((df_set, lab, data_set))
        ...
        if len(df_spread) > 1:
            ...
            if plot:
                ...
                ax_dev.scatter(df_spread['baseline'],
                               df_spread['curvature'] - line_xint1(df_spread['baseline'], *popt), color=df_spread['color'])
